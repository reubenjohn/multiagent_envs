...
class AgentOb(object):
	def __init__(self, dist_behind, dist_forward, vel):
		self.dist_behind = dist_behind
...
class AgentState(object):
	...
	def action_update(self, action: AgentAction, prev: 'AgentState', nxt: 'AgentState', env: 'RoundAbout'):
		...
		self.vel = np.clip(self.vel + acc, 0, free_space_in_front)

		if free_space_in_front < 1e-3:
			self.vel = 0

		displacement = min(u, free_space_in_front)
...
class RoundAbout(MultiAgentEnv2d):
	...
	def __init__(self, n_agents: int, occupancy_ratio: float = .5, window_width: int = 720):
		super().__init__(n_agents, window_width, window_width)
		self.scale = window_width / 3
		self.focus = Point(.4, 0)

		self.max_steps = 8 * n_agents
		self.car_arc_ratio = occupancy_ratio / n_agents
		self.max_vel = (1 - occupancy_ratio) * 2 / (2 * n_agents)
		self.max_acc = (1 - occupancy_ratio) * 2 / (2 * n_agents) ** 2 * RoundAbout.OVERSPEED_COLLISION_RATIO
	...
	def display_agent(self, index, agent_state: AgentState, agent_debug: AgentDebug):
		...
		cv2.putText(self.img,
					'[%d] %.2fm @ %.1f%%' % (index, agent_state.pos, 100 * agent_state.vel / self.max_vel),
					fr_point, cv2.FONT_HERSHEY_SIMPLEX, self.window_transform_float(.002), agent_debug.color)
	...
	def observations(self) -> List[AgentOb]:
		return [
			AgentOb((curr.pos - prev.pos) % 1, (nxt.pos - curr.pos) % 1, curr.vel)
